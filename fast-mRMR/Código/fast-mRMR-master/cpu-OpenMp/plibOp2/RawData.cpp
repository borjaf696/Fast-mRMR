...
void RawData::loadData() {
	uint i, j;
	t_data buffer[1];
	data = (t_data*) malloc(featuresSize* sizeof(t_data) * datasize);
    ...
    //vectGet = (t_feature)malloc((datasize+1)*sizeof(t_data));
}
...
void RawData::calculateVR() {
	...
	t_data dataReaded;
	uint vr;
	valuesRange = (uint*) malloc(featuresSize* sizeof(uint));
	...
	printf("Pues efectivamente fallo aqui\n");
}
...
t_data * RawData::getDataArray(){
    return this->data;
}

void RawData::setValuesRangeArray(uint *ranges){
    for (uint i = 0; i < featuresSize; i++)
        valuesRange[i] = ranges[i];
}

void RawData::setValuesDataArray(t_data *ranges){
    for (uint i = 0; i < featuresSize; i++)
        for (uint j = 0; j < datasize; j++)
            data[i*datasize+j] = ranges[i*datasize+j];
}
...
t_feature RawData::getVectGet(){
	return vectGet;
}

void RawData::setVectGet(int index){
    for (uint i = 0; i <  datasize; i++)
        vectGet[i] = data[index * datasize + i];
	vectGet[datasize] = getValuesRange(index);
}

void RawData::setVectGet2(t_feature vect, uint range){
	vectGet = vect;
	vectGet[datasize] = range;
}
/*Rellenamos de manera manual los valores de RawData*/
void RawData::setValues(Envio send){
	featuresSize = send.featuresSize;
	datasize = send.dataSize;
	this->valuesRange = (uint*)malloc(featuresSize*sizeof(uint));
	this->data = (t_data*) malloc(featuresSize* sizeof(t_data) * datasize);
	//vectGet = (t_feature)malloc((datasize+1)*sizeof(t_data));
}

void RawData::setValues(t_data *datos) {
    for (uint i= 0; i < datasize; i++)
        for (uint j = 0; j < featuresSize; j++)
            data[j*datasize+i] = datos[j*datasize+i];
}
