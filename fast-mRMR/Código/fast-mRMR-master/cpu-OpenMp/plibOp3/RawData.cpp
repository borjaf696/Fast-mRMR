...
void RawData::loadData() {
	...
	data2 = (t_data*) calloc(featuresSize2, sizeof(t_data) * datasize);
	//Avanza 8 bits en el fichero de datos, para saltar metaData
	fseek(dataFile,inicio+8,0);
        Timer tiempo1;
	tiempo1.start();
	//printf("Number of features %d number of datasize %d\n",featuresSize,datasize);
...
void RawData::calculateVR() {
	...
	#pragma omp parallel for
		...
		vr = 0;
		for (j = 0; j < datasize; j++){
			dataReaded = data2[i*datasize + j];
			if (dataReaded > vr)
				vr++;
		}
		valuesRange2[i] = vr + 1;
	}
	double tiempoGas = tiempo2.stop();
	//printf("Tiempo 2: %f\n",tiempoGas);
}
