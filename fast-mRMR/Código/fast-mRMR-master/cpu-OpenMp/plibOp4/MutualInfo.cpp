...
//Calculates the mutual information between the given features. -> Cálculo de la informacion mutua entre dos features.
...
//Another way to work
double MutualInfo::get(t_feature featureInt, uint featureIndex2, double* prob1,int featureIndex) {
	...
	t_prob marginalY = 0;
	t_prob division = 0;
	JointProb jointProbTable = JointProb(rawData, featureInt, featureIndex2, featureIndex);
	for (i = 0; i < range1; i++) {
		for (j = 0; j < range2; j++) {
			jointProb = jointProbTable.getProb(i, j);
			if (jointProb != 0) {
