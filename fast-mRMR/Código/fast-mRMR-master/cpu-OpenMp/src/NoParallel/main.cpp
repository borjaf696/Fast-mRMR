...
int main(int argc, char* argv[]) {
	...
	tm.start();
	RawData rawData = RawData(opts.file); //Paralelizable -> LoadData y CalculateVR -> ambas

	ProbTable prob = ProbTable(rawData); //Paralelizable -> Calculate -> Fr / numThreads -> Optimizacion devolver las probabilidades
	...
	while (selectedFeatures.size() < rawData.getFeaturesSize() - 1 /*-1 because class is discarded*/ and selectedFeatures.size() < opts.selectedFeatures) {
		...
		for (j = 0; j < rawData.getFeaturesSize(); ++j) {
			...
			if (find(selectedFeatures.begin(), selectedFeatures.end(), j) == selectedFeatures.end() && j != opts.classIndex) {
