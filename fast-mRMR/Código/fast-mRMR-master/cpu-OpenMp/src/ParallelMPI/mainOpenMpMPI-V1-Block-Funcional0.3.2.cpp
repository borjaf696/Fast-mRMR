...
#include "../../plibOp4/JointProb.h"
#include "../../plibOp4/MutualInfo.h"
#include "../../plibOp4/utils.h"
...
int main(int argc, char* argv[]) {
    ...
    if (vectInteres2 == NULL){
	printf("Memory limit exceed vectInteres2 %d\n",mpi_rank);
	exit(3);
    }
    if (proceso > mpi_size)
        proceso =mpi_size - 1;
	...
	printf("VectInteres %d %d\n",vectInteres2[dataSizes],vectInteres2[dataSizes+1]);
        int pos = dataSizes+2;
   ...
   MPI_Bcast(vectInteres2,dataSizes+2+sizeof(double)*256,MPI_CHAR, proceso, MPI_COMM_WORLD);
    ...
    #pragma omp parallel for schedule(dynamic)
        for (uint l = startPoint; l < startPoint + sizePerProcess; ++l) {
            relevancesParciales[l-startPoint] = mutualInfo.get(vectInteres2,l-startPoint, probVector,-1);
...
newFeatureIndex= 329;
    ...
    free(probVector);
    free(vectInteres2);
    vectInteres2 = (t_feature)malloc((dataSizes+2+sizeof(double)*256)*sizeof(t_data));
	...
	if (!mpi_rank)
		for (int i = 0; i< dataSizes;++i)
			if (vectInteres2[i] > 25 || vectInteres2[i] < 0)
				printf("Valor extraño %d\n", vectInteres2[i]);
	...
	mrmr = -std::numeric_limits<double>::infinity();
		...
		/*if (mpi_rank == 0){
			...
			for (uint i=0;i < dataSizes;++i){
				if (i % 1000000 == 0)
					printf("VectInteres2 %d\n",vectInteres2[i]);
			}
			printf("Yep\n");
			printf("VectInteres2 %d %d\n",vectInteres2[dataSizes],vectInteres2[dataSizes+1]);
			printf("Yip\n");
		}*/
                //printf("Thread %d\n",omp_get_thread_num());
                if (visited[j-startPoint] == 0 && j != opts.classIndex) {
                    //redundancesParciales[j-startPoint] += mutualInfo.get(lastFeatureInde
                    redundancesParciales[j-startPoint] += mutualInfo.get(vectInteres2,j-startPoint,probVector,lastFeatureIndex);
	...
	if (lastFeatureIndex == 329){
		printf("Prob %f %d\n",probVector[0],mpi_rank);
		printf("VectInteres %d %d\n",vectInteres2[1000],mpi_rank);
	}
        etapas++;
    }
