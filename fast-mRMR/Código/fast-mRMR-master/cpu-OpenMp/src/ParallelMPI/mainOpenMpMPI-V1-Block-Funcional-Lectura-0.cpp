...
void typeDefinitionMpi(){
    int numItems = 2;
    MPI_Datatype tipos[numItems] = {MPI_INT,MPI_INT};
    MPI_Aint offsets[numItems];

    int numElements[numItems] = {1,1};

    offsets[0] = offsetof(Envio,featuresSize);
    offsets[1] = offsetof(Envio, dataSize);

    MPI_Type_create_struct(numItems, numElements, offsets, tipos, &rawData_type);
    MPI_Type_commit(&rawData_type);
}
...
int main(int argc, char* argv[]) {
    ...
    options opts;
    uint j = 0;
    uint newFeatureIndex = 0, lastFeatureIndex = 0;
    uint sizePerProcess = 0, sizePerProcessGlobal = 0, *sendSize;
    double mrmr = 0;
    double acum = 0;
    ...
    tm.start();
    //Lectura y recepcion de datos
    RawData rawData;
    Envio send;

    if (!mpi_rank) {
        sendSize = (uint*)malloc(mpi_size*sizeof(uint));
        sizePerProcess = (int)ceil(rawData.getFeaturesSize()/mpi_size);
        for (int i = 0; i < (mpi_size - 1); i++)
            sendSize[i] = sizePerProcess;
        sendSize[mpi_size - 1] = rawData.getFeaturesSize() - (mpi_size - 1) * sizePerProcess;

        //Se envia a cada proceso en el send -> datasize y su featureSize
        rawData = RawData(opts.file);
        /*rawData.setVectGet(opts.classIndex);
        printf("Clase %d VectGet %d NumClass %d %d\n",rawData.getFeature(opts.classIndex)[23],rawData.getVectGet()[23]
            ,opts.classIndex, rawData.getVectGet()[rawData.getDataSize()]);*/
        send.featuresSize = rawData.getFeaturesSize();
        send.dataSize = rawData.getDataSize();
        free(sendSize);
    }else
        rawData = RawData();
    //Enviamos los datos cargados en Master a todos los procesos
    MPI_Bcast(&send, 1, rawData_type, 0, MPI_COMM_WORLD);

    if (mpi_rank)
        rawData.setValues(send);

    MPI_Bcast(rawData.getValuesRangeArray(),rawData.getFeaturesSize(), MPI_INT, 0, MPI_COMM_WORLD);

    if (!mpi_rank)
        for (int i = 1; i < mpi_size; i++)
            MPI_Send(rawData.getFeature(0),rawData.getFeaturesSize()*rawData.getDataSize(), MPI_INT, i, 0, MPI_COMM_WORLD);
    else {
        t_data *datos = (t_data*)malloc(rawData.getFeaturesSize()*rawData.getDataSize()*sizeof(t_data));
        MPI_Recv(datos, rawData.getFeaturesSize() * rawData.getDataSize(), MPI_INT, 0,
                 MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        rawData.setValues(datos);
        free(datos);
    }

    printf("Termine %d: Fs %d  Ds %d Vr %d Value %d\n",mpi_rank, rawData.getFeaturesSize(),
           rawData.getDataSize(),rawData.getValuesRange(5000),rawData.getFeature(100)[2]);

    t_feature featureGet;
    featureGet = (t_feature)malloc((rawData.getDataSize()+2)* sizeof(t_data));
    //Fin de la info basica
    ...
    //Calculo de relevancias parciales a cada proceso. -> Envio del vector de interes numero 1
    //MPI_Bcast(&rawData.getVectGet()[0], rawData.getDataSize()+1, MPI_INT, 0, MPI_COMM_WORLD);
    if (!mpi_rank) {
        for (int i = 0; i < rawData.getDataSize(); i++)
            featureGet[i] = rawData.getFeature(opts.classIndex)[i];
        featureGet[rawData.getDataSize()] = rawData.getValuesRange(opts.classIndex);
        featureGet[rawData.getDataSize()+1] = opts.classIndex;
    }
    MPI_Bcast(featureGet, rawData.getDataSize()+2, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    #pragma omp parallel for
        for (uint l = startPoint; l < startPoint + sizePerProcess; ++l) {
            ...
            relevancesParciales[l-startPoint] = mutualInfo.get(featureGet,l);
    ...
    free(visited);
    free(relevancesParciales);
    free(redundancesParciales);
