...
#include "../../plibOp6/JointProb.h"
#include "../../plibOp6/MutualInfo.h"
#include "../../plibOp6/utils.h"
...
void getMaxRelevance2(double* classRelevances, uint classIndex, uint rawDataSize, uint globalInteresValue, int mpi_rank, double *result, int suma) {
    ...
    for (i = 0; i < rawDataSize ; ++i) {
	if (i+globalInteresValue*mpi_rank+suma == 1 || i+globalInteresValue*mpi_rank+suma == 130)
		printf("%d %f\n", i+globalInteresValue*mpi_rank+suma,classRelevances[i]);
        if (classRelevances[i] > relevance && i != classIndex) {
            relevance = classRelevances[i];
...
int main(int argc, char* argv[]) {
    ...
    if (mpi_rank == 0)
	for (int f = 0 ; f < rawData.getDataSize();++f)
		if (f % 1000000 == 0)
			printf("%d\n",rawData.getFeature(0)[f]);
    ...
    printf("Rawdata values %d\n",rawData.getValuesFeatureStudio());
    if (mpi_rank != proceso)
	...
	printf("Proceso %d inicio %d suma %d\n",mpi_rank,startPoint,suma);
