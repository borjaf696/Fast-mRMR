...
void getMaxRelevance2(double* classRelevances, uint classIndex, uint rawDataSize, int mpi_size, int mpi_rank, double *result) {
    uint i = 0;
    uint newFeature = -1;
    double relevance = 0;
    for (i = 0; i < rawDataSize ; ++i) {
        if (classRelevances[i] > relevance && i != classIndex) {
            relevance = classRelevances[i];
            newFeature = mpi_rank+i*mpi_size;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (mpi_rank < (int)rawData.getFeaturesSize()%mpi_size)
        sizePerProcess++;
    ...
    int startPoint = 0;
    #pragma omp parallel
    for (uint l = startPoint+mpi_rank;l < rawData.getFeaturesSize(); l+=mpi_size){
    ...
    getMaxRelevance2(relevancesParciales, opts.classIndex, iterator, mpi_size, mpi_rank, parInteres);
    ...
    while (etapas < rawData.getFeaturesSize() - 1 /*-1 because class is discarded*/
        ...
        for (j = 0;j < sizePerProcess; ++j){
            uint k = mpi_size*j+mpi_rank;
            if (visited[k] == 0 && k != opts.classIndex) {
                redundancesParciales[j] += mutualInfo.get(lastFeatureIndex, k);
                mrmr = relevancesParciales[j] - (redundancesParciales[j] /(double) etapas);
                if (mrmr > acum) {
                    acum = mrmr;
                    newFeatureIndex = k;
                }
